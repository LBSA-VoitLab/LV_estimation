using DifferentialEquations, DiffEqSensitivity, Plots, DelimitedFiles, BlackBoxOptim, OptimizationBBO, OptimizationFlux, OptimizationOptimisers, Optimization
...
optprob = OptimizationFunction((x,p) -> loss_function(x))
prob = Optimization.OptimizationProblem(optprob, p0,lb=lower, ub = upper)
res = solve(prob, BBO_adaptive_de_rand_1_bin_radiuslimited(),population=100,maxiters=its, progress=true)
